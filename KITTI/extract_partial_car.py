...
for j in range (0, 7480):
    ...
    bboxes = utils.load_3d_boxes(label_path, 'car')
    if len(bboxes != 0):
        if (os.path.exists('output/car/') == False):
            os.makedirs('output/car/points/')
            os.makedirs('output/car/boxes/')
        ...
        for i in range(len(points_flag)):
            ...
            if len(p)>0:
                ...
                pts_name = 'output/{}/points/{}_point_{}'.format('car', n, idx)
                box_name = 'output/{}/boxes/{}_bbox_{}'.format('car', n, idx)
